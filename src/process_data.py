...
def preprocess(labels, tokenized_inputs, do_mask=False):
    result = {}
...
def preprocess_batch(data_batch, tokenizer, do_mask=False):
    ...
    for i, labels in enumerate(all_labels):
        new_labels.append(preprocess(all_labels[i], tokenized_inputs[i]))
...
def prepare_data(dataset, pretrained_model):
    tokenizer = AutoTokenizer.from_pretrained(pretrained_model)
    tokenized_datasets = dataset.map(
        preprocess_batch,
        batched=True,
        fn_kwargs={"tokenizer": tokenizer, 'do_mask': False},
